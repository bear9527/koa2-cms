...
// 新增菜单
const addMenu = async (ctx: any, next: any) => {
  ...
  const cateAddRes = await query(
    INSERT_DATA(
      "ev_menus",
      "title, description, parentId, img",
      `'${title}', '${description}', '${parentId || defaultParentId}', '${img}'`
    )
  );
...
// 查询所有菜单
const getMenus = async (ctx: any) => {
  const { parentId } = ctx.query;
  let menuReapeatRes = null;
  if (parentId) {
    menuReapeatRes = await query(
      GET_TABLE_INFO("ev_menus", ["parentId", "deleted"]),
      [parentId, "0"]
    );
  } else {
    menuReapeatRes = await query(
      GET_TABLE_INFO("ev_menus", "deleted"),
      0
    );
  ...
  let newData = menuReapeatRes.map((item: any) => {
    const preUrl =
...
const addArticle = async (ctx: any, next: any) => {
  ...
  const articleAddRes = await query(
    INSERT_DATA(
      "ev_articles ",
      "title, description, content, state, parentId, articlePic, createDate, createUser",
      `'${bodyData.title}', 
...
const getArticleList = async (ctx: any) => {
  ...
  if (parentId) {
    cateReapeatRes = await query(
      GET_TABLE_INFO("ev_articles", ["parentId", "deleted"]),
      [parentId, "0"]
    );
  } else {
